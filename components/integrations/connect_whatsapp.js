...
export default function ConnectWhatsapp({...props}){
    ...
    const selectWAInstance = async (data)=>{
        ...
        if(data.status=="AVAILABLE"||data.status=="CONNECTING"){
            console.log({user_id:user.user_id , node_index:data.node_index , user_name:user.username , team_id:user.team_id})
            const start = await axios.post(data.url+"/connect" , {user_id:user.user_id , node_index:data.node_index , user_name:user.username , team_id:user.team_id},{ headers:{
