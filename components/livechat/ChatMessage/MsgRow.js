...
export default function MsgRow({msg, isSearch, refProp, replyHandle, confirmReply, confirmForward, ...props}) {
    ...
    const messageType = (msg) => {
        ...
        switch (msg.message_type) {
            ...
            case "sticker":
                return (<div className={"msg_type_sticker" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}>
                    {props.replyMsg == msg.timestamp ?
                        <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
                    <img id={msg.timestamp} onClick={replyclick} value={msg.timestamp} className={"imageBox"}
                         src={msg.media_url}/>
                </div>);
            ...
            case "image":
                if (msg.body) {
                    return <div id={msg.timestamp} onClick={replyclick} value={msg.timestamp}
                                className={"msg_type_imageCaption" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                        <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
                        ...
                        <div id={msg.timestamp} onClick={replyclick} value={msg.timestamp}
                             className={"imageMessage"}>{msg.body}</div>
                    </div>
                }
                return <div id={msg.timestamp} onClick={replyclick}
                            className={"msg_type_image" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                    <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}<img id={msg.timestamp}
                                                                                                   className={"imageBox"}
                                                                                                   src={msg.media_url}/>
                </div>;
            // case "imageCaption":
            ...
            //                         <img id={msg.timestamp} className={"imageBox"} src={JSON.parse(captionJson).url} onClick={()=>{setImageOpen(!isImageOpen)}}/>
            //                         <div id={msg.timestamp} onClick={replyclick}  className={"imageMessage"}>{JSON.parse(captionJson).message}</div>
            //                     </div>

            case "VIDEO":
            case "video":
                return <div id={msg.timestamp}
                            className={"msg_type_video" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                    <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
            ...
            case "voice" :
                return <div id={msg.timestamp} onClick={replyclick} value={msg.timestamp}
                            className={"msg_type_voice " + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                    <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
                    <audio id={msg.timestamp} onClick={replyclick} value={msg.timestamp} className={"voice_detail"}
                           controls src={msg.media_url}></audio>
                </div>;
            case "FILE":
            case "file":
            case "document":
                return <div id={msg.timestamp} onClick={replyclick} value={msg.timestamp}
                            className={"msg_type_attachment" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                    <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
                    <div id={msg.timestamp} onClick={replyclick} className={"attachmentBox"}>
                        <svg id={msg.timestamp} onClick={replyclick} xmlns="http://www.w3.org/2000/svg" width="35"
                        ...
                        {/*<a id={msg.timestamp}  className={"attachmentBody"} href={msg.media_url} download>{msg.body||msg.media_url.slice(0,25)+"..."}</a>*/}
            ...
            case "link":
                return <div id={msg.timestamp} onClick={replyclick}
                            className={"url_body" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                    <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
                    <div className={"url_box"}>
                        ...
                        <div id={msg.timestamp} onClick={replyclick} value={msg.timestamp}
                             className={"msg_type_url" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                            <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
            ...
            default:
                return (
                    ...
                    // :
                    <div id={msg.timestamp} onClick={replyclick} value={msg.timestamp}
                         className={isSearch ? "msg_body_highligh" : "msg_body" + (props.replyMsg == msg.timestamp ? " replyActive" : "")}> {props.replyMsg == msg.timestamp ?
                        <Reply confirmReply={confirmReply} confirmForward={confirmForward}/> : ""}
    ...
    return (
        <div className={"msg_row"} id={msg.timestamp}>
            <div className={msg.from_me ? "msg_from_me" : "msg_from_other"}>
                ...
                {msg.sender && msg.from_me && <div
                    className={"msg_timestamp"}>{msg.sign_name}&ensp; {new Date(parseInt(msg.timestamp * 1000)).toDateString('en-US', {
                    hour: '2-digit',
                    minute: '2-digit'
                ...
                {msg.sender && !msg.from_me && <div
                    className={"msg_timestamp"}> &ensp;  {new Date(parseInt(msg.timestamp * 1000)).toLocaleTimeString('en-US', {
                    hour: '2-digit',
                    minute: '2-digit'
                })} </div>}
            </div>
